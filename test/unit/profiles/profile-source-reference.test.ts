...
describe('profile source reference codec', () => {
  const reference = { schemaVersion: 1 as const, source: 'source' };
  it('encodes exact source-only schema-v1 bytes', () => {
    expect(encodeProfileSourceReference(reference)).toBe('{\n  "schemaVersion": 1,\n  "source": "source"\n}\n');
    expect(decodeProfileSourceReference(JSON.parse(encodeProfileSourceReference(reference)), 'source')).toEqual(reference);
  });
  it('rejects provider fields and mismatched path identity', () => {
    expect(() => decodeProfileSourceReference({ ...reference, provider: 'legacy' })).toThrow(/exactly/u);
    expect(() => decodeProfileSourceReference(reference, 'other')).toThrow(/does not match/u);
  });
  ...
  it('fails the complete reference index closed on unsafe and non-directory profile entries', async () => {
    ...
    const result = await findReferencingProfiles(directory.path('home'), 'source');
  ...
  it('captures all source counts and diagnostics in one bulk reference-index snapshot', async () => {
    ...
    const alpha = encodeProfileSourceReference({ schemaVersion: 1, source: 'alpha' });
    const beta = encodeProfileSourceReference({ schemaVersion: 1, source: 'beta' });
    await directory.write('home/profiles/first/sources/alpha.json', alpha);
    await directory.write('home/profiles/second/sources/alpha.json', alpha);
    await directory.write('home/profiles/second/sources/beta.json', beta);

    const result = await captureProfileSourceReferenceBulkIndex(directory.path('home'));

    expect(result.diagnostics).toEqual([]);
    expect(result.profileIdsBySource.get(profileSourceReferenceKey('alpha'))).toEqual(['first', 'second']);
    expect(result.profileIdsBySource.get(profileSourceReferenceKey('beta'))).toEqual(['second']);
  });

  it('reports old nested provider directories as invalid reference entries', async () => {
    const directory = await createTempDirectory('bazframe-reference-namespace-'); directories.push(directory);
    await directory.write('home/profiles/focused/sources/provider/source.json', encodeProfileSourceReference(reference));
  ...
  it('rejects symlinked profiles and profile ancestors before exact reads', async () => {
    ...
    await directory.write('outside/profiles/focused/sources/source.json', encodeProfileSourceReference(reference));
    await directory.mkdir('home');
    await symlink(directory.path('outside/profiles'), directory.path('home/profiles'));
    await expect(readProfileSourceReferenceSnapshot(directory.path('home'), 'focused', 'source'))
    ...
    await expect(readProfileSourceReferenceSnapshot(directory.path('home'), 'focused', 'source'))
      .rejects.toMatchObject({ code: 'SOURCE_REFERENCE_INVALID' });
    await expect(readProfileSourceReferenceSnapshot(directory.path('home'), 'Unsafe', 'source'))
      .rejects.toMatchObject({ code: 'SOURCE_REFERENCE_INVALID' });
  });
  ...
  it('rejects symlinked namespace ancestors and final links', async () => {
    ...
    await directory.write('outside/source.json', encodeProfileSourceReference(reference));
    ...
    await symlink(directory.path('outside/source.json'), directory.path('home/profiles/focused/sources/source.json'));
    await expect(readProfileSourceReferenceSnapshot(directory.path('home'), 'focused', 'source'))
      .rejects.toMatchObject({ code: 'SOURCE_REFERENCE_INVALID' });
  });
  ...
  it('rejects malformed bytes and non-regular files without blocking', async () => {
    ...
    const path = await directory.write('home/profiles/focused/sources/source.json', new Uint8Array([0xff]));
    await expect(readProfileSourceReferenceSnapshot(directory.path('home'), 'focused', 'source'))
      .rejects.toMatchObject({ code: 'SOURCE_REFERENCE_INVALID' });
    await directory.write('home/profiles/focused/sources/source.json', '{');
    await expect(readProfileSourceReferenceSnapshot(directory.path('home'), 'focused', 'source'))
      .rejects.toMatchObject({ code: 'SOURCE_REFERENCE_INVALID' });
