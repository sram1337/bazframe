...
async function runWindowsTransactionJournalSmoke(packageRoot, parent) {
  ...
  const { readWindowsPhysicalFileSnapshot } = await load('profiles/win32-profile-selection');
  const { enumerateWindowsPhysicalDirectory } = await load('skills/added-skill-platform-services');
  ...
  privateState.admitWindowsPhysicalDirectory(backend, parent);
  ...
  const snapshot = (file) => readWindowsPhysicalFileSnapshot(backend, file, 1024 * 1024);
  const names = async () => (await enumerateWindowsPhysicalDirectory(backend, root, 32768)).names;
