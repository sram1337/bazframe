...
import { admitWindowsPhysicalDirectory, ensureWindowsPrivateDirectoryPath, isValidWindowsPathComponent } from '../state/win32-private-directory.js';
...
const same = (a: WindowsPathInspection, b: WindowsPathInspection) => a.object.volumeIdentity === b.object.volumeIdentity && a.object.fileId === b.object.fileId && a.object.creationTime === b.object.creationTime;
...
export function createWindowsOwnedFileServices(backend: BazframeWin32NativeBackend & BazframeWin32LockBackend, options: WindowsFileServiceOptions & {
  ...
  async function entries(directory: string): Promise<string[]> {
    try { return (await enumerateWindowsPhysicalDirectory(backend, directory, PROFILE_PORTABILITY_PRODUCTION_LIMITS.stagingEntries)).names; }
  ...
  async function absent(file: string): Promise<boolean> {
    ...
    let names;
    try { names = (await enumerateWindowsPhysicalDirectory(backend, parent, PROFILE_PORTABILITY_PRODUCTION_LIMITS.stagingEntries)).names; }
  ...
  async function snapshot(file: string, max: number): Promise<Snapshot> {
    ...
    const value = await readWindowsPhysicalFileSnapshot(backend, file, max);
      ...
      return withWindowsOperationLock({ backend, lockRootPath: win32.join(home, 'locks'), lockComponent: options.lockComponent, details: { command, target: home }, io: options.lockIo }, async (authority) => {
        ...
        const result = await operation({
          ...
          async detach(file, expected, maxBytes) {
            ...
            const parentPath = win32.dirname(file), parent = admitWindowsPhysicalDirectory(backend, parentPath);
            ...
            assertHeld();
            if (!same(parent, admitWindowsPhysicalDirectory(backend, parentPath))) throw refused('parent changed; retain both leaves');
