...
import { admitWindowsPhysicalDirectory, admitWindowsPhysicalFile, createWindowsPrivateDirectory, ensureWindowsPrivateDirectoryPath, isValidWindowsPathComponent } from '../state/win32-private-directory.js';
...
export function createWindowsReadyResourceServices(backend: BazframeWin32NativeBackend & BazframeWin32LockBackend, options: WindowsProfileLifecycleOptions & { resolvePackageExecutable?: typeof resolvePackageExecutable; scope?: { home: string; authority: OperationMutationAuthority; stateAuthority: { assertHeld(): void } }; sourceReads?: import('../profile-publishing/physical-profile-closure.js').PhysicalProfileReadServices } = {}): SkillCollectionLifecycleServices {
  ...
  async function admitProfile(home: string, profileId: string): Promise<ResourceRootIdentity> {
    ...
    const before = identity(admitWindowsPhysicalDirectory(backend, root));
    ...
    try { admitWindowsPhysicalDirectory(backend, win32.join(root, 'skills')); }
    catch (error) { if (errorCode(error) !== 'WINDOWS_NATIVE_PATH_NOT_FOUND' || (await enumerateWindowsPhysicalDirectory(backend, root, maximum)).names.some((name) => key(name) === key('skills'))) throw error; }
    const after = identity(admitWindowsPhysicalDirectory(backend, root));
    if (!sameResourceIdentity(before, after)) throw refused('profile root changed');
    return { ...after, root };
  }
  const physical = () => createWindowsPhysicalReads(backend, undefined, {});
  ...
  async function canonical(path: string): Promise<string> {
    ...
    const directory = await physical().openDirectory(path, path);
  ...
  async function readFile(path: string, max: number) {
    const before = backend.inspectPath(path);
    const bytes = (await physical().readFile(path, max)).bytes;
  ...
  async function retainedFile(path: string, name: string): Promise<boolean> { if (!isRetainedResourceFile(name)) return false; admitWindowsPhysicalFile(backend, path); return true; }
  ...
  function resolver(): SkillCollectionResolverEffects {
    return {
      ...
      async canonical(path) { if (await absent(path)) throw refused('missing physical resource'); const value = backend.inspectPath(path); if (!value.ancestryReparseFree || value.object.reparseTag !== null || value.canonicalPath !== value.volume.canonicalVolumeGuidPath + path.slice(3)) throw refused('aliased physical resource'); return path; },
      async stat(path) { if (await absent(path)) throw Object.assign(new Error('absent'), { code: 'ENOENT' }); const value = backend.inspectPath(path); return { isDirectory: () => value.kind === 'directory', isFile: () => value.kind === 'regular-file', isSymbolicLink: () => value.object.reparseTag !== null }; },
  ...
  async function stateSnapshot(path: string): Promise<{ digest: string; bytes?: Buffer; inspection?: WindowsPathInspection }> {
    ...
    const snapshot = await readWindowsPhysicalFileSnapshot(backend, path, boundedStateJsonBytes());
  ...
  function scoped(home?: string, authority?: OperationMutationAuthority, stateAuthority?: { assertHeld(): void }): SkillCollectionLifecycleServices {
    ...
    function snapshotDependencies() {
      const publication: SkillSnapshotPublicationEffects = {
        ...
        sourceReads: { joinPath: win32.join, physical: options.sourceReads ?? physical(), logicalExecutable: false },
    ...
    const preparation: SkillCollectionPreparationEffects = {
      ...
      manifestAbsent: (root) => absent(win32.join(root, PACKAGE_MANIFEST)),
      async readManifest(root, lower = {}) { const policy = packageLimitPolicy(lower), path = win32.join(root, PACKAGE_MANIFEST); const { bytes, ...proof } = await readFile(path, policy.maxManifestBytes); return { ...proof, path, manifest: decodePackageManifest(json(bytes), policy), contentSha256: hash(bytes) }; }
    };
    ...
    return {
      ...
      async detach(path, root, expected) {
        ...
        assertAuthority();
        const afterParent = admitWindowsPhysicalDirectory(backend, parentPath);
        if (!sameResourceIdentity(identity(parent), identity(afterParent))) throw refused('detach parent changed; retained ambiguity');
        const current = await stateSnapshot(path), moved = await stateSnapshot(retainedPath);
        if (current.inspection === undefined && moved.inspection !== undefined && moved.bytes?.equals(initial.bytes) && sameResourceIdentity(identity(moved.inspection), identity(initial.inspection)) && moved.inspection.object.attributes === initial.inspection.object.attributes) { assertAuthority(); return; }
