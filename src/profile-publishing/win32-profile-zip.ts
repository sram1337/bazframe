...
import { admitWindowsPhysicalDirectory, admitWindowsPhysicalFile, createWindowsPrivateFile, createWindowsPrivateDirectory, isValidWindowsPathComponent } from '../state/win32-private-directory.js';
...
export function openWindowsProfileZipRangeSource(backend: BazframeWin32NativeBackend, path: string, maximum: number): ProfileZipRangeSource {
  const inspect = () => inspectFile(backend, path);
...
export function createWindowsProfileZipEffects(backend: BazframeWin32NativeBackend, options: { io?: WindowsProfileZipIo; temporaryRoot?: string } = {}): { readZip: typeof readProfileZip; writeZip: typeof writeProfileZip } {
  ...
  const readZip: typeof readProfileZip = async (input, readOptions = {}) => {
    ...
    createWindowsPrivateDirectory(backend, temporaryRoot, stageName);
    ...
    const copied = inspectFile(backend, path);
  ...
  const writeZip: typeof writeProfileZip = async (output, profile, blobs, writeOptions = {}) => {
    ...
    const created = createWindowsPrivateFile(backend, parentPath, candidateName);
    ...
    const candidate = await optionalOutput(candidatePath, policy.maxAggregateBytes);
    ...
    if (!sameObject(parent, admitWindowsPhysicalDirectory(backend, parentPath)) || !sameSnapshot(expected, await optionalOutput(destination, policy.maxAggregateBytes)) || !sameSnapshot(candidate, await optionalOutput(candidatePath, policy.maxAggregateBytes))) throw refused('ZIP publication baseline changed');
    let rejected = false;
    try {
    ...
    try {
      if (!sameObject(parent, admitWindowsPhysicalDirectory(backend, parentPath))) throw refused('ZIP parent changed');
      const after = await optionalOutput(destination, policy.maxAggregateBytes), retained = await optionalOutput(candidatePath, policy.maxAggregateBytes);
  ...
  async function optionalOutput(path: string, maximum: number) {
    const parent = win32.dirname(path), name = win32.basename(path);
    const namespace = await enumerateWindowsPhysicalDirectory(backend, parent, capturedProfileLimitPolicy().maxEntries);
    ...
    const inspection = inspectFile(backend, path);
    const source = openWindowsProfileZipRangeSource(backend, path, maximum);
    const sha256 = await hashSource(source);
    if (!exact(inspection, inspectFile(backend, path))) throw refused('ZIP output changed');
...
function inspectFile(backend: BazframeWin32NativeBackend, path: string): WindowsPathInspection {
  canonicalPath(path);
  const parent = admitWindowsPhysicalDirectory(backend, win32.dirname(path));
  const value = admitWindowsPhysicalFile(backend, path);
  if (value.kind !== 'regular-file' || value.object.directory || value.object.reparseTag !== null || value.object.deletePending || !value.ancestryReparseFree || value.canonicalPath !== win32.join(parent.canonicalPath, win32.basename(path))) throw refused('ZIP file is unsafe or aliased');
  return value;
}
