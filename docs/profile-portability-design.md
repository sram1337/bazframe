...
> Status: approved product target; unexposed foundations implemented, vertical slice pending
>
...
Profile portability moves one Bazframe profile between machines as a reviewable declaration. It does not copy `BAZFRAME_HOME` or bundle provider trees.
...
- managed direct individually added Skill memberships;
...
A path-free managed Git requirement is exactly the managed record's normalized `remote`, credential-free `fetchUrl`, `branch`, and exact `revision`, copied from existing managed provenance without a network lookup. It excludes checkout roots and transport preference. Library and package records themselves store only their source directory and snapshot digest; managed metadata comes from the separate existing managed Git provenance record.
...
2. The artifact contains no provider checkout, library/package snapshot, cache, adapter state, policy, favorite, recovery state, current active profile, credential, project state, or Pi state.
3. Managed Git identity is locked to normalized remote identity, credential-free fetch URL, branch, and exact revision copied from existing managed provenance. Export performs no network lookup, and import never substitutes the branch's current head for the exported revision.
...
The managed source fields come from the resource's existing managed Git provenance record. They are not reconstructed from a library record and export does not contact the remote. In Stage 1 a local library or any package reference blocks export rather than producing a resource.
Schema rules:
...
- Managed source keys are exactly `type`, `remote`, `fetchUrl`, `branch`, and `revision`.
- Managed identities use the existing credential-free source and managed-record validators. Embedded credentials, query strings, fragments, local/file sources, option-shaped values, and ambiguous forms remain invalid.
- Managed entries omit checkout roots, transport preference, authentication, environment, and timestamps.
...
The artifact binds source identity, not publisher trust. Healthy local direct Skills whose profile links exactly match their added Skills are not portable resources: export emits a deterministic terminal-safe warning, omits them from the exported profile state, and records their IDs in `omittedLocalSkills`. There is no local Skill mapping. A future local library or package mapping records less reproducibility than a managed exact revision because the importer explicitly chooses the source directory on the destination machine. Package builds can also produce environment-dependent bytes. Exported snapshot digests are therefore not portable identity and are omitted.
...
- healthy matching global resources, snapshots, and managed provenance where applicable.
...
- a direct Skill or collection under Bazframe's managed Git paths without matching managed provenance;
...
For each direct Skill, export checks the same-ID added Skill in `(default)`. A Skill acquired through managed Git is included with its existing normalized, credential-free provenance identity. A Skill from a healthy local source directory is omitted, its ID is recorded in `profile.omittedLocalSkills`, and export emits a deterministic terminal-safe warning naming the sorted omissions. A broken or mismatched link, or a Skill under a managed path without matching provenance, causes export to fail without publishing an artifact. No local Skill mapping is emitted.

A profile's library or package reference names a global library or package. Export verifies that the matching global record and activated immutable snapshot are both present and healthy. For a managed Git resource, export copies source identity from its existing managed provenance; the library/package record itself supplies only its source directory and snapshot digest. In Stage 1, a local library or any package reference causes export to fail without publishing an artifact. Later stages may emit `localMapping` for local libraries and packages, but the artifact never contains a source-machine filesystem path.
...
Running `profile import` without `--dry-run` authorizes the acquisition work and Bazframe-owned state writes shown in the inspection plan. It does not authorize collisions, replacement, branch-head substitution, changing the current active profile, or package execution beyond the package consent rules below.
...
### Exact-revision managed Git

The current ordinary managed add path chooses the selected branch head. Portability adds a dedicated locked-revision acquisition path:
...
8. validate and commit through the normal resource-specific managed lifecycle.
...
This requires factoring managed acquisition so ordinary add retains branch-head behavior while portability supplies an exact required revision.
...
- package ID and managed identity or local mapped root;
...
Interactive confirmation is a literal approval that defaults to decline. `--yes` is accepted only when `--dry-run` is absent; it authorizes each exact revalidated package report noninteractively but does not suppress the report. Consent is bound to package ID, candidate-root identity, managed revision or local mapping, and exact manifest snapshot. A declined build cleans identity-proven managed staging and executes nothing. An exact reused package does not rebuild and needs no build consent.

The package preparation lifecycle gains a `beforePackageBuild` callback invoked inside preparation immediately before `executeBuild`. For managed packages it revalidates the canonical root identity, clean checkout, detached `HEAD`, origin, `refs/remotes/origin/<branch>`, revision, and authorized manifest. For local mappings it revalidates the canonical path/physical root identity and authorized manifest. The callback and spawn remain adjacent inside the lifecycle; the accepted final race against a non-cooperating same-user writer still applies.

Import reuses the rest of the existing package lifecycle and expected-manifest seam. Filesystem rollback covers Bazframe-owned staging and records, not arbitrary effects of provider code.
...
| Managed resource has exact remote, fetch URL, branch, revision, checkout, registration/record, and healthy snapshot where applicable | Reuse without network or build |
| Any managed identity field differs | Fail; do not update or repoint |
...
| Same-kind ID has unexpected physical, broken, malformed, recovering, or unmanaged occupancy | Fail |
| A Skill registration link does not exactly target the required managed or mapped root, or a library/package record is linked | Fail |
...
Cross-machine snapshot digest equality is not required. The target snapshot must be healthy and its provider identity must satisfy the artifact. An environment-dependent package build may legitimately produce different output, so the product does not claim reproducible package artifacts.
...
Implementation reuses the existing safe-ID, instruction-byte, traversal, Skill-discovery, terminal-safety, managed-Git, lock, atomic-file, and identity-verification contracts. It must additionally:
...
- use private staging and identity-proven cleanup;
- avoid printing instruction bodies during ordinary inspection and errors.

Existing authoritative limits apply where their semantics match, including the current effective-instruction and Skill traversal limits. The new manifest/resource/acquisition limits must be centralized, documented, and tested below/at/above their approved values before implementation can ship. Git transfer limits need defined timeout/abort, child termination, staging quarantine, and identity-proven cleanup behavior; where a hard portable pre-transfer byte cap cannot be guaranteed, the supported transport/platform contract must state the enforceable approximation and fail safely on timeout or post-transfer budget excess. This design does not invent numerical limits without implementation evidence. An unbounded untrusted checkout, snapshot, package manifest, or process duration is a release blocker.
...
interface ResourceImportPlan {
  ...
  id: string;
  sourceType: 'managedGit' | 'localMapping';
...
- a path-free managed identity decoder sharing existing validators;
...
### Stage 1: managed, build-free vertical slice
...
- export/import managed direct Skills and managed libraries;
...
- add exact-revision managed acquisition;
...
- add managed and locally mapped packages;
...
Portability is not described as complete until managed direct Skills, explicit local direct-Skill omissions, managed/local libraries, packages, and all approved resource/acquisition bounds ship.
...
- unmanaged, foreign, broken, or mismatched direct Skills;
- unhealthy records, snapshots, managed provenance, and recovery states;
...
- every managed identity mismatch;
...
- Raw home, profile-tree, provider-tree, checkout, snapshot, or archive transfer.
...
- New managed transports beyond the existing managed Git source contract.
