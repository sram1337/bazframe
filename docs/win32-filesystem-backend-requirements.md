...
Contract v6 adds one native read-only fact: exact no-follow inspection of an NTFS directory junction, its owner/DACL, its reparse payload, and the admitted physical target's lossless identity. The final and reopened link handles require `READ_CONTROL`; link identity and security are compared across both the inner and repeated native inspections. Directory symlinks, junction chains, dangling targets, malformed payloads, foreign link ACLs, other reparse tags, and changed parent/link/target observations fail closed. Native code neither creates nor removes links.

The internal TypeScript composition owns safe canonical Skill IDs, Windows component validity, owner-private physical-parent admission, the inherited effective owner-private ACL policy for the link object, independent physical-target admission, direct-child and direct-target comparison, and immediate parent/link/target revalidation. The link's current-user SID must equal the admitted parent's. It creates only with Node's no-replace `symlink(target, membershipPath, 'junction')` and removes only with `unlink(membershipPath)`. A failed pathname call is reconciled from the revalidated absent/exact/changed tuple; uncertainty remains visible and never authorizes target removal. The supported-platform final-pathname race remains disclosed.
