...
Windows follows the shared macOS/Linux lifecycle: authorized logical removal detaches the named non-active profile from the live namespace. Detached profiles, backups and ambiguous transaction state remain owner-private, with an honest retained-state result. Physical recursive deletion is not part of successful profile removal or a prerequisite for Windows support. External Skill targets and other unowned content remain untouched; membership unlink removes only the admitted link leaf.
...
The Bazframe-owned Windows x64 Rust/N-API backend supplies local-NTFS/path admission, lossless identity and same-handle security inspection, bounded whole-file and ranged reads/enumeration, protected private directory/file creation, no-replace sibling directory and regular-file publication, same-volume two-admitted-parent directory movement, cooperating-writer locks, exact private junction inspection/creation, and native contract 7 read-only contained final-file editor inspection. These capabilities address measured Node/Windows gaps. TypeScript owns product policy and composes the narrow native operations; implementation topology is not a permanent product requirement.

Internal onboarding, healthy local Skill membership and managed activation/current/switching exist behind injected services. Public CLI/Pi dispatch remains closed. Onboarding retains its directory-publication/recovery composition; the connected product uses shared lifecycle policy rather than nesting transaction engines.
...
ZIP byte-source classification is separate from managed locality/ownership admission: ordinary disk files and the specific cloud reparse family can be copied without trusting source ancestry ownership. Mapped network byte sources retain the source exception; generic reparses, device aliases, pipes and non-regular/multiple-link files still refuse. Stage/output admission stays local/private as applicable, copying is bounded and drained, and the canonical parser sees only the reconciled local copy. An operation-bound shared view rechecks the actual branded authority after asynchronous reads settle. Native cloud/provider, mapping, ACL and sharing behavior remains unqualified.

Internal W7–W8 application-plan Tasks 1–12 now compose one lazy shared backend identity into discovery/policy, existing CLI/TUI branches, rich status and actual shipped Pi handlers, private installer-bound runtime integrity, discovery-safe retained uninstall, the controlled launcher and both editors. Native contract 7 editor proof admits and revalidates the complete resolved parent chain without reading editor-target content. Independent source/host reviews accept the application corrections, including the final shared imported-plus-physical collision inputs for library/package listings. Prose/JSON lists, status and actual Pi handlers have connected host regressions for whole-collection withholding and nonmutating reads. The final narrow review verified its exact five-file delta and preservation of the other 386 paths. Evidence is under `/tmp/bazframe-windows-completion.AH77qw/` in `application-consumer-corrections-review.md`, `application-security-corrections-review.md`, `application-final-fix.md` and `application-final-fix-review.md`. Native installed-product outcomes remain unqualified; public gates stay closed.
...
- logical removal with private retained profiles/backups, external-target preservation and retained ambiguity on drift, bounds or sharing failures;
...
- all profile lifecycle, selection, favorite, editor, active-profile, and removal/private-retention paths;
...
The full current-product gate remains required. This checklist sequences the outcomes in sections 2, 6 and 8; it is not execution authorization. Use shared codecs, consent, identity, views and lifecycle policy, adding Windows mechanisms only for concrete gaps.

Checked W0–W8 implementation bullets record the independently reviewed internal source/host work summarized in section 7, not fresh execution receipts or native acceptance. Native/installed/terminal qualification bullets remain open. W9's documentation preparation below is independently reviewed; candidate execution remains pending. W10 support guidance and publication remain pending.

Development uses one implementation writer, focused changed-behavior host tests and independent review as appropriate to the task. Native tests are needed for genuine platform outcomes when authorized; source/packed qualification is not a prerequisite for every local increment. Full installed-product and release validation remain separate gates.
...
- [x] Adapt immutable blob/tree publication and validation to accepted Windows file identities, single-link/private storage and logical executable metadata; do not inherit a hard-link publication strategy that violates the accepted Windows closure contract.
...
- [x] Exercise healthy/changed/missing/foreign/malformed resource paths with host shared-engine tests; connect the internal resource services without changing public support gates.
...
- [x] Extend shared recovery composition to candidate swaps and sidecar-only publication; unknown or ambiguous state remains private and retained.
...
- [x] Internally compose guarded ordinary removal: generated-empty or fresh lossless preview confirmation, active/stale refusal, absent favorite cleanup, exact private retained sibling and untouched added-Skill targets. Host tests use real shared phase policy and V2 storage.
...
- [ ] Natively qualify logical removal and private retained profiles/backups without undoing successful detachment or deleting external Skill targets. Preserve the shared resource cleanup semantics; do not add automatic snapshot/blob GC.
...
- [x] Use private admitted local acquisition/publication workspaces with the existing Git environment, prompting/hooks and credential rules; retain workspaces whenever process-tree settlement is unproved. Git workspace deletion is not a release prerequisite.
...
- [x] Implement installer-bound internal Windows bootstrap and shared Pi handlers behind the unchanged default gate; host handler/bootstrap evidence is not public default routing.
...
- [x] Implement the shared deprecated-launcher branch: dry-run, forwarded arguments, refusals, child exit/signal and cleanup uncertainty; no pre-acceptance launch.
...
- [x] Independently accept the W9 surface/evidence map below. Every current command/TUI operation is mapped with direct, indirect and missing evidence distinguished; the public gate remains closed.
- [x] Independently accept the full-surface unreleased-candidate procedure below, including actual public CLI and default-Pi routing. Candidate execution and support transition remain separately authorized, pending work.
...
**How to use this map.** Anchors below are inspected host tests/scripts, not passed receipts. `test/unit/cli/platform-support.test.ts` retains the compile-time exhaustive list of all 49 reachable command shapes; `profiles-overview` is an unreachable retained union member, not another command. `test/unit/cli/parse-argv.test.ts` covers grammar/migration; `test/unit/cli/json-output.test.ts` and `test/unit/cli/command-results.test.ts` anchor protocol/presentation. These common anchors apply to every CLI row, including Windows refusal before service construction. Windows-named host tests use injected/synthetic filesystem/native boundaries; host Git or real shared handlers do not turn them into native runs.
...
| `profile add`, `duplicate`, `rename`, `remove` | `test/unit/profiles/win32-profile-provisioning.test.ts`; `test/unit/profile-publishing/win32-profile-lifecycle.test.ts`; `test/unit/profile-publishing/profile-managed-lifecycle.test.ts`; `test/unit/profile-publishing/profile-recovery.test.ts`; `test/integration/profile-management-cli.test.ts` | Inactive creation/duplicate, rich-profile identities, active rename, guarded removal/`--force`, active-removal refusal, favorite preservation, retained private state and untouched external targets; native phase interruption and contention pending. |
...
| `tui` | TUI map below; `test/unit/cli/tui-command.test.ts`; `test/integration/tui-cli-state-agreement.test.ts`, `test/integration/tui-pty.test.ts`; terminal scripts | Gate before TUI access; JSON and noninteractive refusal; real terminal entry/error/restoration. Both installed names and Windows Terminal pending. |
...
The Pi evidence has three distinct boundaries: `test/unit/adapters/pi-artifact.test.ts` tests shared handlers, but its Windows default-gate case **rewrites the platform declaration and poisons `resolveState` in the source under test**; it is only synthetic gate evidence. `test/unit/adapters/pi/win32-bootstrap.test.ts` instead transpiles exact installer-produced bytes and injects import/native-I/O boundaries, but invokes the internal bootstrap, not actual default dispatch. `test/unit/core/win32-native.test.ts` and `test/unit/cli/platform-support.test.ts` separately test loader/CLI refusal without rewriting the shipped Pi source. None proves installed Windows default Pi. No production Pi extraction or new test-only default wrapper is needed for this preparation.

### W9 unreleased-candidate procedure (reviewed; execution pending)

This is preparation for later authorized execution, not a support bypass or permission to run native/remote workloads. Closed-mainline regression, candidate qualification, support admission and npm publication are separate decisions.

1. **Preserve closed mainline.** `src/core/platform-support.ts` continues to refuse Windows before `src/cli/run-cli.ts` constructs services. The default export in `artifacts/pi/bazframe.ts` continues to register blocked handlers before binding/state/native I/O. Ordinary source builds/tests omit native bytes; normal push CI remains foundation/Added-Skill-slice evidence, not W9. No flag, environment override, source patch at test time, or internal-dispatch wrapper opens the gate now.
2. **Authorize and isolate the later candidate.** Identify the clean base commit, separate candidate checkout, ordinary-user native Windows x64 machine/token and local-NTFS roots, concrete Node/npm/Pi versions, native shell/npm launchers, local/global prefixes, Windows Terminal/font/locale, reviewed ZIP/Git/build fixtures, and explicitly authorized disposable remote accounts/repositories. Do not reuse personal installations or the unsettled historical remote workspace. The existing Node 22.19.0 job does not qualify all of `>=22.19.0`; untested claimed versions remain gaps. Pi must be stable in `>=0.84.4 <0.85.0 || >=0.85.1`.
3. **Review an explicit candidate-only routing delta.** The central CLI transition must let the real `src/cli.ts` entrypoint reach existing production Windows application composition. Both bins remain `dist/cli.js`. Separately, the actual default Pi extension must await installer-bound Windows bootstrap, validate its reference/package/runtime/native bytes before runtime import, and register the real shared handlers. Today that bootstrap is only exported as `createWindowsBoundPiAdapterForInternalTesting`; merely opening the CLI gate or calling that export from a harness does not change the default extension. Review the necessary production routing and bootstrap failure reporting (absent/stale binding before import), preserving non-Windows behavior. Any factoring needed belongs to that later reviewed delta, not this preparation. Identify/commit the candidate only with separate authorization; do not merge its gate transition into main before full acceptance or add a user-settable support switch.
4. **Bind exact source, bytes and environment.** Build from the designated candidate using pinned Rust/MSVC/native sources and the existing build-only `foundation-evidence` packaging mechanism; that mode grants neither release assembly nor support admission. Record commit/tree and reviewed main-to-candidate delta, package/native version and contract, Node/npm/Rust/MSVC versions, native SHA-256, whole-tarball SHA-256 and inventory. Never relabel an older native binary. Designate and retain one tarball, then install that same file everywhere without repacking. Record resolved dependency versions/lock or installed-tree inventory and Pi executable path/package/version: a Bazframe tarball alone does not bind transitively resolved dependencies or an external `PI_BIN`.
5. **Reuse qualification infrastructure truthfully.** `scripts/run-win32-qualification.mjs` runs foundation and Added-Skill product-slice workloads, not this matrix. Foundation-v6/all-62 and product-v3/all-72 (including historical `70834d5`/run `34081175304`) cannot certify W9 or new source. `scripts/verify-win32-added-skill-evidence.mjs` requires the closed gate, `releaseAdmission: "not-authorized"`, and `windowsSupportClaim: false`; its unchanged product-v3 schema cannot validate a gate-open candidate. Before full-surface native automation, independently review the necessary extension of the existing workload/verifier/qualification integration to bind actual W9 observations. That prerequisite remains pending; do not hand-edit receipts, reuse old counts as thresholds or introduce a parallel certification service. Negative closed-gate evidence stays bound to exact closed source/artifacts.
