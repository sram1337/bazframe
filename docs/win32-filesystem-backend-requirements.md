...
- exact schema-v6 native evidence and later product wiring for junction-only Skill-membership creation, inspection, and link-only removal;
...
This slice is not imported by current catalog, profile, publishing, Pi, CLI, or TUI services and does not bypass the central Windows gate. Exact schema-v6 source-tree and packed-install native evidence is required before the internal capability is accepted.
