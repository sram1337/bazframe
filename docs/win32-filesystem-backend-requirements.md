...
Currently, `src/core/platform-support.ts` refuses Windows command effects before CLI service construction, and the default Pi extension registers blocked handlers before binding, native initialization or state I/O. Internal services allow focused development and tests.

Internal capability refusal must occur before mutation, transaction recovery, stale-lock reclamation, external process launch, adapter/policy writes, or TUI state access that could trigger recovery. The current packaged Pi extension applies its platform check before startup, reload, or `/bazframe info` reads or recovery. The gate applies consistently to local/global installs and both executable names. Help/version/syntax paths bypass filesystem initialization and remain platform-neutral.
...
- installed Pi extension platform refusal before managed-state access, then accepted `pi` and `pi -nc` context restoration/provenance/order, enabled/disabled policy, collision aliases/failure withholding, `/bazframe info`, and `/bazframe reload`;
...
- [x] Implement installer-bound internal Windows bootstrap and shared Pi handlers through internal composition; host handler/bootstrap evidence does not exercise default dispatch.
...
- [ ] Verify missing/corrupt/wrong-target/version/ABI native-artifact diagnostics and storage refusal cases; preserve platform-neutral help/version/syntax and the CLI/Pi platform-refusal tests.
...
3. **Review production routing.** The central CLI transition must let the real `src/cli.ts` entrypoint reach existing production Windows application composition. Both bins remain `dist/cli.js`. Separately, the actual default Pi extension must await installer-bound Windows bootstrap, validate its reference/package/runtime/native bytes before runtime import, and register the real shared handlers. Today that bootstrap is only exported as `createWindowsBoundPiAdapterForInternalTesting`; merely opening the CLI gate or calling that export from a harness does not change the default extension. Review production routing and bootstrap failure reporting (absent/stale binding before import) through the normal development workflow, preserving non-Windows behavior and capability checks. Installed acceptance must use those production entrypoints rather than a test-only switch.
...
5. **Reuse qualification infrastructure truthfully.** `scripts/run-win32-qualification.mjs` runs foundation and Added-Skill product-slice workloads, not this matrix. Current tooling uses native contract 8, foundation-v7 and product-v4 schemas. Historical foundation-v6/all-62 and product-v3/all-72 receipts (including `70834d5`/run `34081175304`) cannot certify W9 or new source. `scripts/verify-win32-added-skill-evidence.mjs` expects platform refusal, `releaseAdmission: "not-authorized"`, and `windowsSupportClaim: false`; it does not validate production Windows routing. Before full-surface native automation, independently review the necessary extension of the existing workload/verifier/qualification integration to bind actual W9 observations. That prerequisite remains pending; do not hand-edit receipts, reuse old counts as thresholds or introduce a parallel certification service. Platform-refusal evidence stays bound to the exact source/artifacts that produced it.
