...
This slice is not imported by current catalog, profile, publishing, Pi, CLI, or TUI services and does not bypass the central Windows gate. Exact schema-v6 source-tree and packed-install native evidence passed in run `33936692831`; the internal capability is accepted only as a foundation for later product wiring, not as a public Windows capability.
