...
Contract-v2 facts cannot prove a directory closure: path inspection and stable read require an already-known path, while pathname-only enumeration cannot prove that a complete name set came from the admitted directory handle or bind each listed child to a later opened child. The one new native export supplies only that unavailable fact. TypeScript retains recursion, deterministic ordering and collision policy, aggregate limits, owner-private and single-link child admission, opened-child reconciliation, file hashing, two-pass logical closure comparison, and final private-root revalidation.
