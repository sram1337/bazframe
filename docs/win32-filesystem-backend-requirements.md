...
### 7.6 Pending internal managed activation/current/active-Skill composition
...
Independent source/security review and evidence re-review pass; this milestone remains **unaccepted pending authorized native source/packed execution**. Separate product schema v3 retains the 44 accepted onboarding/Skill booleans and adds 28 activation/current/active-target observations (72 total), including real junction projection, selection replacement identity, active/explicit isolation, killed-owner/contention, real sharing denial, exact no/after-effect/substitution outcomes, and interruption before replacement, after replacement, and before lifecycle return. Failure receipts retain fixed, non-identifying stage/code allowlists. Foundation-v6 schema, native Rust/API/binary inventory, release admission, and public Windows gates are unchanged. No default CLI/TUI/Pi caller constructs this dependency. macOS/Linux defaults remain on their existing readers, locks, and writer.

Local fake-backed and supported-host validation has passed; native Windows/PowerShell execution is unavailable on this macOS host, so neither this source nor its new evidence schema implies native acceptance. The accepted onboarding evidence in section 7.5 remains the latest product acceptance. Node/NTFS file-replacement and sharing behavior must be proved by the separate source/packed harness before advancing acceptance; no new native operation has been proposed or added.
...
Final promotion requires all three successful jobs, revalidates the exact foundation input and both strictly bound single product receipts, and requires v3/all-72 pair parity before copying original verified bytes into the unchanged seven-file foundation and two-file product final success artifacts. Only final foundation upload outputs reach the reusable release consumer. Failed/cancelled diagnostics remain sanitized receipt/toolchain allowlists, never payloads. Local PowerShell still builds/packs/installs once, but runs all four workloads sequentially foundation-source→foundation-packed→product-source→product-packed, stops on the first failed lane, and adds no deadline or coordinator-created hierarchy. Host independent-root real npm reconstruction passed with inert native bytes; Windows reconstruction, timing, source/packed native receipts and independent acceptance remain pending. Isolation is not established as a fix for either earlier lock/directory refusal. Public Windows support and release gates remain closed.
