...
`bazframe profile skill add [--profile <profile>] <skill>` resolves the named added Skill and creates a parallel membership link under `profiles/<id>/skills/` directly to the same canonical source target—never a link chain. On macOS/Linux this is an absolute symlink; the planned native-Windows representation must have the same exact direct-target behavior and pass its reparse, immediate revalidation, and link-only removal gates. Removal accepts only an exact parallel link matching the current `(default)` entry. Both operations are idempotent, preserve source content, and take the state lock before the profile membership lock. Bazframe refuses physical, relative, foreign, mismatched, or non-added entries. A missing add target offers bounded suggestions from valid added Skills.
