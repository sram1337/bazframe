...
`bazify` converts one explicitly selected local physical Skill into a new provider-owned package with a Bazframe-compatible build manifest. Its dependency-free Node script uses `./bazframe/` for working files, defaults the package/repository ID to the unchanged Skill name and the package destination to `~/<id>`, never appends a Bazframe suffix, never overwrites, rejects links/special entries and obvious credential material, copies in lexical order, generates the exact compatibility/build/lifecycle scaffold, and validates through `packages add` under a disposable `BAZFRAME_HOME`. Source-specific installation, setup, provenance, licensing, privacy, and redistribution review remain agent/user decisions. The Skill tracks that work through an applicable local todo system under `./bazframe/`, or through a lightweight temporary checklist there that is removed when resolved. GitHub publication is a separate private-only operation fixed to `github.com`; its dry run emits a token binding the authenticated account, repository, canonical package path, and publishable-byte digest. Publication requires that exact token plus `--yes` after the Skill obtains explicit confirmation unless the invoking request already supplied `-y`/`--yes`, rejects drift through final Git-index verification, initializes one new local Git repository, and runs fixed shell-free `git`/`gh` argv. It never exports a profile, mutates the user's Bazframe catalog/profile, publishes during create/validate/build/install, overwrites an existing local/remote repository, or claims that secret scanning proves safety.
