...
`bazframe profile skill add [--profile <profile>] <skill>` resolves the named added Skill and creates a parallel membership link under `profiles/<id>/skills/` directly to the same canonical source target—never a link chain. On macOS/Linux this is an absolute symlink; internal Windows services use an exactly admitted private junction. Internal onboarding and managed activation/current/switching use the same lifecycle engines. Removal accepts only an exact parallel link matching the current `(default)` entry. Both operations are idempotent, preserve source content, and take the state lock before the profile membership lock. Bazframe refuses physical, relative, foreign, mismatched, or non-added entries. A missing add target offers bounded suggestions from valid added Skills.
