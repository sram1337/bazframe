...
The public CLI now selects lazy Windows application services on native Windows x64, retaining package-local native contract/version/target checks and accepted local-NTFS admission. Node.js 22.19.0 or newer is required. This source transition does not change the already-published beta.3 or establish final installed-package/release admission; [the Windows requirements](win32-filesystem-backend-requirements.md) record those separate gates.
...
`profile duplicate <source> <new>`, `profile rename <old> <new>`, `profile use <profile>`, and forced removal use the managed profile lifecycle. Duplicate assigns a new profile identity, clears publication linkage, preserves shared imported identities, and derives new identities for copied profile-local Skills. Rename preserves profile and resource identities. Removal remains local-only and always refuses the active profile. Authorized removal detaches the named owned profile from the live namespace and retains existing state with its protection unchanged, matching the shared lifecycle; immediate physical deletion is not promised. These mutations use operation locks, whole-profile candidate swaps or durable journals, and startup recovery; they apply to Pi on the next startup or `/reload`.

A profile's resources are managed canonically with `bazframe profile skill|library|package add|remove`. Omission targets the active profile; an explicit `--profile` changes that profile without changing or requiring active selection. Ordinary registrations retain their unqualified selectors. Independently imported instances use dynamic `profile/name` qualification only while needed to resolve collisions; membership stores stable identity rather than the display alias. Imported collection child Skills appear in the shared catalog and runtime projection. `profile skill|library|package list` remains active-profile-only. Pi remains authoritative for full runtime validation. Packs are outside the current scope.

### Profile sharing and portability

The ZIP/GitHub profile-sharing lifecycle replaces the historical Stage 3 directory artifact on macOS, Linux and the Windows x64/local-NTFS boundary. `profile export` writes a deterministic ZIP; `profile publish` versions the same canonical capture at exact commits reachable from GitHub `refs/heads/main`. Capture includes ready local Skill, library, and package-artifact bytes and never builds. Exact remote references remain references unless `--bundle-remote` is selected. A physical direct Skill carries only the non-identifying `sourceForm: "profile-local"` marker so import can restore its physical form while deriving a new local identity. Transport excludes profile instance IDs, ownership IDs, source-machine paths, and publication linkage.

Capture shows the exact bounded file preview and applies finite credential-filename exclusions; this is not secret scanning, and user-authored instructions still require review. ZIP import is independent; Git import remains linked to its canonical origin. A name collision offers the first free suffix, explicit overwrite, or cancellation. `--yes` accepts routine confirmation and the safe suffix; only `--overwrite` authorizes replacement or discarded local changes.

Dry-run uses private OS-temporary retained workspace state and performs no `BAZFRAME_HOME` mutation, transaction recovery, build, or login. Only settled clone/fetch unavailability may make initial profile creation incomplete. Existing-profile update, version selection, overwrite, and repair are atomic and cannot degrade the installed profile. Skills, libraries, and packages materialize in that order with packages last; package build consent is bound to the exact revalidated identity and build report. Exact healthy cached resources are reused offline without build or consent.

Git updates and `profile version list|use` preserve exact reachable revisions. Publication uses exact expected-old leases, private-first creation, explicit public visibility confirmation, durable local/remote intent, and predicate-based recovery. Managed duplicate, rename, use, membership, and removal preserve their documented stable identity rules. Imported and profile-local resources are projected to Pi from validated physical or immutable artifact roots.

Production Git runs with an isolated allowlisted environment, disabled prompting/hooks, bounded output, and no shell. Public Git imports can use unauthenticated strict Git; publication and private import require `gh`. Git/GitHub and exact-revision acquisition workspaces are retained under private random paths because portable Node cannot prove safe recursive reclamation; a separately approved reclamation design is required. Excessively long Unix-socket lock paths fail closed on supported platforms.

[`profile-publishing-design.md`](profile-publishing-design.md) defines product behavior, [`profile-publishing-engineering.md`](profile-publishing-engineering.md) defines the shipped macOS/Linux implementation contract, [`win32-filesystem-backend-requirements.md`](win32-filesystem-backend-requirements.md) defines the approved Windows support, storage, safety and full-product acceptance outcomes, and [`profile-portability-design.md`](profile-portability-design.md) is the historical Stage 1–3 record. The first platform boundary is Windows x64 with managed state on accepted local NTFS; network-backed Bazframe home and staging remain unsupported, while bounded ZIP copying and Git network transport into private local staging are permitted.
...
Bazframe's built-in added Skill catalog is `(default)`, rooted at `<BAZFRAME_HOME>/skills`. On macOS/Linux, each entry `<skill>` is an absolute symlink to one canonical physical Skill root. Windows uses the exactly validated directory-symlink or junction representation defined in [`win32-filesystem-backend-requirements.md`](win32-filesystem-backend-requirements.md), reached through the public lazy Windows application composition. The canonical target basename and declared frontmatter `name` must exactly equal the safe skill ID. Local-source targets remain externally owned; remote Git sources use an exact recorded checkout under `<BAZFRAME_HOME>/providers/git/checkouts/skill/`. Source changes are live on the next Pi startup or `/bazframe reload`.

`bazframe skill add <absolute-root-or-git-source>` adds one structurally validated Skill to `(default)`. An absolute path retains the existing local-source ownership contract. A remote Git source acquires a root Skill at its stable Bazframe-managed checkout path and records exact provenance before catalog publication. Local Skill-first setup validates source/name and canonical overlap through admitted existing ancestry before protected home/lock bootstrap, then revalidates full overlap/provenance under authority before membership publication. Read-only and absent removal do not bootstrap state. The same verified target is idempotently current; occupied, broken, malformed, dirty, or mismatched state is rejected. Paths inside `BAZFRAME_HOME` are accepted only when exact remote Git source provenance proves their identity. `bazframe skill remove <skill>` removes only the catalog membership link, is absent-idempotent, and refuses every valid profile dependency or unverifiable reference index. A broken absolute registration remains removable only after all literal-target profile memberships are gone. These lifecycles hold the global state lock and revalidate the parent, link, and exact target immediately before publication or unlink. On both the shipped macOS/Linux path and the Windows path, link-only removal never deletes the target but retains the documented final-syscall race against a non-cooperating process with the same user authority.
...
`bazframe profile skill add [--profile <profile>] <skill>` resolves the named added Skill and creates a parallel membership link under `profiles/<id>/skills/` directly to the same canonical source target—never a link chain. On macOS/Linux this is an absolute symlink; Windows services use an exactly admitted private junction. Onboarding and managed activation/current/switching use the same lifecycle engines. Removal accepts only an exact parallel link matching the current `(default)` entry. Both operations are idempotent, preserve source content, and take the state lock before the profile membership lock. Bazframe refuses physical, relative, foreign, mismatched, or non-added entries. A missing add target offers bounded suggestions from valid added Skills.

Existing physical profile Skill directories and readable direct Skill references remain runtime-readable independently of catalog registration. Windows admits exact absolute directory symlinks and junctions to physical local targets, with bounded matching Skill definitions and repeated link/target checks. Relative, UNC, chained, and unknown reparses remain unsupported on Windows. A direct reference without a matching catalog target has no catalog identity, managed ownership, or attachable selector; ordinary profile discovery supplies it to runtime, not the managed resource namespace. Membership mutations still require exact catalog agreement. Updates to Skills from remote Git sources preserve the stable target path, so catalog and profile links continue to agree. Skills from remote Git sources are edited upstream and activated with `skill update`. There is no source-specific environment reader, release migration, or copy fallback. The only approved Windows link alternative is the exact membership representation in [`win32-filesystem-backend-requirements.md`](win32-filesystem-backend-requirements.md); its installed behavior is covered by the Windows acceptance matrix. Libraries and packages remain separate immutable-snapshot objects with whole-object profile references.
...
Old `sources/`, profile `sources/`, profile `source-units/`, `source-snapshots/`, `bazframe-source.json`, and `sourceUnitRoot` state is inert pre-alpha content. Bazframe provides no resource reader, alias, migration, or fallback for it. Ordinary profile view/use preserves the exact physical `source-units/` root and binds its directory identity without reading its contents. Unknown profile roots still refuse closure capture. Export, lifecycle, candidate, and recovery captures retain their strict resource and root policies; ordinary read/use evidence does not authorize transport or mutation.
...
| Owner | Resources |
|---|---|
| User | profile content and instructions, library/package source choices, explicit selection/build consent, and source inputs; `profile remove --force` authorizes Bazframe to detach only the named non-active owned profile from the live namespace, retaining private state |
...
The live profile-sharing grammar is:

```text
bazframe profile export [--profile <profile>] [--output <zip>] [--overwrite] [--bundle-remote]
bazframe profile publish [--profile <profile>] [--public | --private] [--bundle-remote] [-y | --yes]
bazframe profile import [--commit <commit>] [--dry-run] [--overwrite] [-y | --yes] <zip | git:user/repository>
bazframe profile update [--profile <profile>] [--overwrite]
bazframe profile version list [--profile <profile>]
bazframe profile version use <commit> [--profile <profile>] [--overwrite]
```

`bzf` is an equivalent installed executable name. The historical directory artifact, `--as`, and `--map` are not accepted. `--yes` authorizes routine confirmations and safe defaults; it never implies `--overwrite`.

`profile exportable [--profile <profile>] [--bundle-remote]` is designed but not implemented. It will be a deterministic, effect-free query using the production capture and archive-validation paths. It will report exportability, Bazframe resource-closure requirements, and separate advisory environment signals without invoking an LLM. The full contract is in [`profile-publishing-design.md`](profile-publishing-design.md).
...
One exact `--json` before Pi's literal `--` selects the CLI-only protocol. Profile export, publish, import, update, and version commands emit strict schema v2 success, refusal, or error documents with authoritative mutation effects; refusals and usage exit 2. Unrelated commands retain schema v1 `{schemaVersion:1,ok,...}` behavior. Every supported JSON invocation writes exactly one bounded newline-terminated document to stdout; diagnostics exclude stacks, causes, terminal controls, credentials, and private managed or temporary paths. Dry-run reports exact all-false mutation effects. Status attention remains successful schema-v1 JSON with exit 3. Help, version, TUI, external editors, and deprecated Pi reject JSON before side effects. JSON package-process streams route to parent stderr while stdin remains inherited.
...
For adapter installation, `--force` repairs a drifted artifact only when a valid Bazframe ownership manifest identifies the destination. For `profile remove`, `--force` separately authorizes removal of the named non-active owned profile from the live namespace, with retained existing state rather than immediate recursive deletion; it never extends authority to symlink targets.
...
Shared reads admit existing data by their physical identity, containment, kind, access, bounds and stability requirements, not OS ownership or a permission template. New sensitive output is private from first visibility (POSIX creation modes or the protected Windows creation descriptor). Windows admission does not repair existing ACLs. Profile rename and retained-backup namespace moves preserve the moved object's protection; replacement with a private fresh candidate does not preserve the old ACL at that path. POSIX managed-directory write preparation can still restrict existing directory modes; this correction does not change it. Ordinary stable hardlinked input/editor targets are accepted; operation-specific owned writes and locks retain their isolation rules. Product/catalog authorization is not an exact UID/SID-owner test.
...
- implementing the designed deterministic `profile exportable` query, then extending profile sharing only through explicit first-class resource, transport, and reclamation decisions backed by the same bounded and atomic contracts;
...
[`profile-publishing-design.md`](profile-publishing-design.md) and [`profile-publishing-engineering.md`](profile-publishing-engineering.md) record the shipped profile-sharing contract and implementation. [`profile-portability-design.md`](profile-portability-design.md) retains the historical Stage 1–3 record. [`pi-adapter-production-design.md`](pi-adapter-production-design.md) records the Pi production baseline and lifecycle rules.
