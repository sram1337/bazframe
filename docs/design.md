...
[`profile-publishing-design.md`](profile-publishing-design.md) defines product behavior, [`profile-publishing-engineering.md`](profile-publishing-engineering.md) defines the shipped macOS/Linux implementation contract, [`win32-filesystem-backend-requirements.md`](win32-filesystem-backend-requirements.md) defines the approved outcome-parity native-Windows path and its in-progress Bazframe-owned native foundation, and [`profile-portability-design.md`](profile-portability-design.md) is the historical Stage 1–3 record. Native Windows remains unsupported until the complete current CLI, runtime, adapter, editor, reclamation, and TUI surface passes one installed-package acceptance gate through both executables where applicable. The first platform boundary is Windows x64 with managed state on accepted local NTFS; network-backed Bazframe home and staging remain unsupported, while bounded ZIP copying and Git network transport into private local staging are permitted.
...
`bazframe profile skill add [--profile <profile>] <skill>` resolves the named added Skill and creates a parallel membership link under `profiles/<id>/skills/` directly to the same canonical source target—never a link chain. On macOS/Linux this is an absolute symlink; the internal native-Windows junction path has passed its foundation reparse, immediate-revalidation, and link-only-removal gate, and its first explicitly injected healthy local-Skill product lifecycle passed separate source/packed evidence. Subsequent internal milestones passed absent-home bootstrap and inactive profile add/list at `092c159` in run `33951808091`, then actual managed activation/current/switching and active-target healthy local-Skill membership at `70834d5` in run `34081175304`. The activation source/packed product receipts pass all 72 observations, with independently verified artifacts and unchanged foundation-v6 evidence. The subsequent shared-operation-authority and diagnostic refactor passed the same internal source/packed contracts at `f7f5614` in run `34149772588`; its earlier native read refusal did not recur, and the cause remains unestablished. These internal milestones do not authorize public Windows support. Removal accepts only an exact parallel link matching the current `(default)` entry. Both operations are idempotent, preserve source content, and take the state lock before the profile membership lock. Bazframe refuses physical, relative, foreign, mismatched, or non-added entries. A missing add target offers bounded suggestions from valid added Skills.

Existing physical profile Skill directories remain runtime-readable. On macOS/Linux, foreign links also remain runtime-readable but are not managed by the membership commands; native Windows diagnoses every non-membership reparse entry as unsupported. Updates to Skills from remote Git sources preserve the stable target path, so catalog and profile links continue to agree. Skills from remote Git sources are edited upstream and activated with `skill update`. There is no source-specific environment reader, release migration, or copy fallback. The only approved Windows link alternative is the exact membership representation in [`win32-filesystem-backend-requirements.md`](win32-filesystem-backend-requirements.md); it must pass as part of the complete Windows product gate. Libraries and packages remain separate immutable-snapshot objects with whole-object profile references.
