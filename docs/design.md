...
Stage 2 profile export and import are live on macOS and Linux. A reviewable export directory contains canonical `bazframe-profile.json` plus the profile's physical `profile/AGENTS.md`; it is not a raw home/profile copy, archive, source bundle, or snapshot transfer. The declaration captures portable profile contents and resource dependencies plus required sorted `omittedLocalSkills`. Only direct Skills acquired from remote Git sources are included. A healthy local direct Skill whose profile link exactly matches its same-ID added Skill produces a deterministic terminal-safe warning, is omitted, and has its ID permanently recorded; direct Skills have no local mapping. A broken or mismatched profile Skill link, or a Skill under a Bazframe-managed checkout path without matching provenance, causes export to fail without publishing an artifact. Source composition validation still includes omitted direct Skills. Whole-library references remain exact resources and may come from remote Git or a healthy local source. A healthy local library exports path-free as exactly `{ "type": "localMapping" }`, with no source-machine root or snapshot digest. Every package reference remains blocked until Stage 3. A path-free remote Git requirement is the normalized `remote`, credential-free `fetchUrl`, `branch`, and exact `revision` copied without network lookup from existing remote Git provenance; library/package records themselves store only source directory and digest.
...
Multi-resource import is resumable forward rather than globally atomic. A healthy resource committed before a later failure remains globally available and possibly unreferenced; the profile is not visible until every dependency and the complete prospective composition validate. Package builds retain exact manifest/argv reporting and explicit unsandboxed-execution authorization, and arbitrary source-process side effects are not rollbackable. Remote Git import must acquire the exported historical revision exactly and fails rather than substituting current branch head.

[`profile-portability-design.md`](profile-portability-design.md) defines the artifact schema, included profile state and dependencies, explicit omissions, local-library/package mappings, exact-revision acquisition, inspection/execution flow, collision matrix, package consent, partial-failure behavior, staged delivery, and validation gates. The production CLI exposes Stage 2 export and inspect-first import. It supports direct Skills from remote Git, libraries from remote Git or explicit local mappings, and required local-Skill omissions; packages remain Stage 3. The artifact caps remain 1 MiB for the canonical manifest, 1,024 total profile entries, 256 resources, and 1,024 streamed entries per profile namespace. Skill snapshots stream enumeration and file copy/hash under fixed 4 MiB manifest, 8,192-entry, depth-32, 4,096-path-byte, 64 MiB per-file, and 512 MiB aggregate-file ceilings.

Export includes healthy direct Skills and libraries acquired from remote Git, exports a healthy local library only as path-free `localMapping`, omits and records healthy local direct Skills, blocks every package reference, and warns the user to review exported `profile/AGENTS.md` before sharing. Import reports a complete plan before effects, supports side-effect-free `--dry-run`, creates or exactly reuses recorded remote Git revisions, and read-only inspects one explicit absolute mapping per local library. The mapping's physical canonical root must have the exact library ID as basename and must not overlap another mapping, the artifact, or `BAZFRAME_HOME`. Missing mappings block. Execution uses ordinary build-free library creation or exact same-root reuse and never overwrites, updates, or repoints a resource. Active selection remains unchanged, earlier committed resources survive later failure and converge through exact reuse on retry, and collection children never enter `(default)`. Local direct-Skill mappings, packages and `--yes`, Windows publication, and full portability remain unavailable. Portability is complete only after package portability and the remaining approved platform/full acceptance gates ship.
...
The live Stage 2 portability grammar is:
...
`--map` is repeatable and supplies one explicit source directory for each artifact-declared local library. Local direct Skills cannot be mapped. Packages and `--yes` remain unavailable until Stage 3, as defined in `profile-portability-design.md`.
