...
[`profile-publishing-design.md`](profile-publishing-design.md) defines product behavior, [`profile-publishing-engineering.md`](profile-publishing-engineering.md) defines the shipped macOS/Linux implementation contract, [`win32-filesystem-backend-requirements.md`](win32-filesystem-backend-requirements.md) defines the approved Windows support, storage, safety and full-product acceptance outcomes, and [`profile-portability-design.md`](profile-portability-design.md) is the historical Stage 1–3 record. Native Windows remains unsupported until the complete current CLI, runtime, adapter, editor, and TUI surface passes one installed-package acceptance gate through both executables where applicable. The first platform boundary is Windows x64 with managed state on accepted local NTFS; network-backed Bazframe home and staging remain unsupported, while bounded ZIP copying and Git network transport into private local staging are permitted.
...
`bazframe profile skill add [--profile <profile>] <skill>` resolves the named added Skill and creates a parallel membership link under `profiles/<id>/skills/` directly to the same canonical source target—never a link chain. On macOS/Linux this is an absolute symlink; internal Windows services use an exactly admitted private junction. Internal onboarding and managed activation/current/switching exist, but the full public Windows gate remains closed. Removal accepts only an exact parallel link matching the current `(default)` entry. Both operations are idempotent, preserve source content, and take the state lock before the profile membership lock. Bazframe refuses physical, relative, foreign, mismatched, or non-added entries. A missing add target offers bounded suggestions from valid added Skills.
...
For adapter installation, `--force` repairs a drifted artifact only when a valid Bazframe ownership manifest identifies the destination. For `profile remove`, `--force` separately authorizes removal of the named non-active owned profile from the live namespace, with private retained state rather than immediate recursive deletion; it never extends authority to symlink targets.
