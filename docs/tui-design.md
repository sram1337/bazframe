...
The top-tab focus model is now separate from active-tab and body/pane focus: `Tab` and `Shift+Tab` traverse focus, while `Left`/`Right` or `h`/`l` moves focused-tab selection and `Enter` activates it. Lists accept `j`/`k` as `Down`/`Up`, and the profile editor accepts portable `J`/`K` pane jumps. The reducer now owns persistent offsets for the profile list, Included pane, Available pane, and Skills browser, with stable-row visibility and authoritative shrink/resize clamping. Still open are deeper source-tree behavior; canonical source identity and broken-root semantics for symlinked/retargeted Skillbook roots; editor launch; settings writes; provider move/rename; additional real sources; and Linux, Windows Terminal, SSH, tmux, and manual assistive-technology validation. These gaps preclude a production-ready claim.
...
- The exact package set is runtime `ink@7.1.1` plus `react@19.2.8`, and development `@types/react@19.2.18` plus `ink-testing-library@4.0.0`. The build includes `react-jsx`/`.tsx` support while retaining NodeNext ESM. Runtime imports occur only through the lazy TUI entry path, and the packed application carries its own dependencies rather than relying on Pi's global installation.
...
- validate real-terminal resize, Ctrl+C, error, and restoration paths on Linux and Windows Terminal and through SSH/tmux;
