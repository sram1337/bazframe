...
A tagged release assembles its Windows x64 binary in the same workflow run through the reusable native-foundation workflow. Its foundation receipts retain foundation-only semantics: both source-tree and packed-install receipts must pass while continuing to report `releaseAdmission: "not-authorized"` and `windowsSupportClaim: false`.
...
Only after all three jobs succeed does promotion reauthenticate the exact inputs and verify both foundation-v6/all-62 and product-v3/all-72 receipts, including exact pair parity and source/binary/tarball bindings. It copies the original verified bytes into the unchanged seven-file final foundation artifact and two-file product artifact. Reusable `artifact_id`/`artifact_digest` outputs refer exclusively to the **final** foundation upload; input and failure/cancellation diagnostic artifacts never authorize the downstream release consumer. This topology passed qualification at `70834d5` in run `34081175304`; subsequent commits still need their own complete evidence. It does not establish the cause of earlier product refusals or authorize full Windows support. Current qualification status and the delivery/local-testing checklist are recorded in [the Windows requirements](win32-filesystem-backend-requirements.md#10-delivery-roadmap-and-local-windows-testing).
...
This admits exact binary bytes only for release assembly. Ordinary checkout builds/tests omit native bytes; a separately admitted tagged assembly may include the binary while Windows remains unsupported. Assembly admission is not support admission: both the central CLI and default-Pi Windows gates stay closed until the complete installed-product matrix passes. The first future authorized tag is the end-to-end proof of the same-run workflow wiring.

The [W9 evidence map and unreleased-candidate procedure](win32-filesystem-backend-requirements.md#w9-surfaceevidence-map) distinguish inspected host anchors, historical receipts, current-snapshot regression and pending native/installed qualification. W0–W8 internal source/host work is recorded; W9 map/procedure preparation is independently reviewed; candidate qualification has not been executed. W10 support guidance, trusted-publisher setup/final artifact review and publication remain separate pending outcomes. Closed-mainline validation and a normal main commit/push do not authorize candidate gate opening, manual native/remote execution, tagging or npm publication.

A later separately authorized candidate needs an explicit reviewed CLI **and actual default-Pi** routing transition, exact source/native/tarball/dependency/environment bindings and genuine local/global installed execution through both names. Existing product-v3 receipts require a closed gate and cannot certify that candidate; the existing workload/verifier integration needs a separately reviewed full-surface extension first. Current pack/real-Pi scripts repack and discard fixtures, so their supported-host passes are not exact-candidate or native failure-retention receipts. Follow W9's procedure rather than internal factories, old receipt counts or a runtime bypass. Any later version/source change needs new bindings and affected validation before publication of the exact approved bytes.
